...
if ( open(my $fh => $samples) ) {
	...
	chomp;
	my @row = split(/,/,$_);
#	warn("adding ",$row[$hdr{'Strain'}]," => ",$row[ $hdr{'Species'} ],"\n");
	$samps{ $row[$hdr{'Strain'}] } = $row[ $hdr{'Species'} ];
    }
}
opendir(DIR,$dir) || die $!;
my $first = 1;
foreach my $file ( readdir(DIR) ) {
	...
	my $buscodirspecies = File::Spec->catdir($BUSCO_dir,$stem);

	if ( opendir(BUSCOD,$buscodirspecies) ) {
	    my $buscomodel = $model;
	    foreach my $target ( readdir(BUSCOD) ) {
		if ($target =~ /run_(\S+)/) {
		    $buscomodel = $1;
		    last;
		}
	    ...
	    my $busco_file = File::Spec->catfile($BUSCO_dir,$stem,
						 sprintf("short_summary.specific.%s.%s.txt",$buscomodel,$stem));
#	    warn("buscofile is $busco_file\n");	
...
print join("\t", qw(SampleID Putative_Species), @header), "\n";
foreach my $sp ( sort keys %stats ) {
    my $spsimple = $sp;
    $spsimple =~ s/\.(shovill|AAFTF|NECAT|pilon|medaka)\S*$//;
    print join("\t", $sp, $samps{$spsimple}, map { $stats{$sp}->{$_} || 'NA' } @header), "\n";
}
